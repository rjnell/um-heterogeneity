...
chr3p_normalised = read.csv("digital-pcr/chr3p-normalised.tsv", sep="\t")
chr8q_normalised = read.csv("digital-pcr/chr8q-normalised.tsv", sep="\t")
...
plot(x,y)
abline(0,1)
file = "digital-pcr/figure-cnv-coloured.png"
...
plot(xlim, 
     ...
     yaxs = "i")
xlim = c(0,7.5)
mtext(text = "Classic approach", cex=1.1, col="#333333", side = 1, line = 2.75, at=3.5)
mtext(text = "SNP-based approach", cex=1.1, col="#333333", side = 2, line = 2.5, at=3.5)
...
for (i in 1:nrow(chr3p_cnv_processed)) {
  ...
  if (!is.na(chr3p_cnv_processed[i,2]) & !is.na(chr3p_snp_processed[i,2])) {
    col = "#bfbfbf"
    if (chr3p_normalised[rownames(chr3p_cnv_processed)[i],5] != "normal") {
      col = "#93D095"
    }
    points(chr3p_cnv_processed[i,2], chr3p_snp_processed[i,2], pch=16, cex=0.9, col=col)
  }
}
...
for (i in 1:nrow(chr8q_cnv_processed)) {
  ...
  if (!is.na(chr8q_cnv_processed[i,2]) & !is.na(chr8q_snp_processed[i,2])) {
    col = "#BFBFBF"
    if (chr8q_normalised[rownames(chr8q_cnv_processed)[i],5] != "normal") {
      col = "#F88E86"
    }
    if (stringr::str_detect(chr8q_normalised[rownames(chr8q_cnv_processed)[i],5],"gain")) {
      col = "#F0B27A"
    }
    if (!is.na(lumc_data$Chr8q_multiallelic[which(lumc_data$Tumor_ID == ids[i])])) {
      col="#CE6EDF"
    }
...
text(8.8,0.7+yy,labels="amplification (monoallelic)", xpd=T, col="#333333",pos=4, cex=1.1)
points(8.6,0+yy, pch=15, cex=.8, col="#CE6EDF", xpd=T)
text(8.8,0+yy,labels="amplification (biallelic)", xpd=T, col="#333333",pos=4, cex=1.1)

rect(0.2,5.2,3.8,6.8,col="white",border=NA)
text(0,6.35,labels="rho = 0.95", xpd=T, col="#333333",pos=4, cex=1.1)
